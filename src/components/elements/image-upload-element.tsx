...
export class ImageUploadElement extends PureComponent<IProps, IState> {
    ...
    componentDidUpdate(prevProps) {
        if (this.props.imageFile !== prevProps.imageFile) {
            this.setState({ imageUrl: URL.createObjectURL(this.props.imageFile) || '' });
        }
    }
    ...
    render() {
        ...
        const uploadButton = (
            <div>
                {imageFile ? <FileDoneOutlined className='upload-icon' style={{ fontSize: "24px" }} /> : <CameraOutlined className='upload-icon' style={{ fontSize: "24px" }} />}
            </div>
        );
        return (
            <div style={{ position: "relative" }} className="avatar-wrapper">
                <ImgCrop aspect={1 / 1} rotate shape="rect" quality={1} modalTitle="Edit Avatar" modalWidth={768}>
                    ...
                    >
                        {uploadButton}
                        {/*  {imageFile ? <FileDoneOutlined /> : <CameraOutlined />} */}
                    </Upload>
                </ImgCrop>
                <ImageUpload isDisabled={isDisabled} beforeUpload={beforeUpload} imageFile={imageFile}/>
                <Image
                    width={200}
                    //height={200}
